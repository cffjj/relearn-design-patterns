...
public class Level3AuthLink extends AuthLink {
    ...
    public Level3AuthLink(String userId, String level) throws ParseException {
        super(userId, level);
    }
    ...
    public AuthInfo doAuth(String userId, String orderId, Date authDate) {
        ...
        if (null == date){
            return new AuthInfo("0001", "单号：", orderId, " 状态：待三级审批负责人 ", levelName);
        }
        AuthLink next = super.next();
        if (null == next){
            return new AuthInfo("0000", "单号：", orderId, " 状态：三级审批负责人完成", " 时间：", f.format(date), " 审批人：", levelName);
        }
        if (authDate.before(beginDate) || authDate.after(endDate)){
            return new AuthInfo("0000","单号：",orderId," 状态：三级审批完成负责人"," 时间：",f.format(date),"审批人：",levelName);
        }
